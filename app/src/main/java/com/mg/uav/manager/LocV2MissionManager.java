...
/**
 * 航线飞行任务V2(Mobile客户端控制)
 */
public class LocV2MissionManager {

    List<WaypointV2> waypointV2List = new ArrayList<>();
    List<WaypointV2Action> waypointV2ActionList = new ArrayList<>();


    private LocV2MissionManager() {
    }

    private static class WayPointMissionManagerHolder {
        private static final LocV2MissionManager INSTANCE = new LocV2MissionManager();
    }

    public static LocV2MissionManager getInstance() {
        return LocV2MissionManager.WayPointMissionManagerHolder.INSTANCE;
    }
...
//            sendErrorMsg2Server(message, "stopMission fail：" + waypointMissionOperator.getCurrentState());
    ...
    public void createWayPointV2Mission(MissionUploadData mission) {
        waypointMissionOperator = DJIApp.getWaypointMissionOperator();
        if (waypointV2List!=null){
            waypointV2List.clear();
        }
        for (int i = 0; i < mission.getWaypoints().size(); i++) {
            double[] latLng = MapConvertUtils.getDJILatLng(Double.parseDouble(mission.getWaypoints().get(i).getLatitude()),
                    Double.parseDouble(mission.getWaypoints().get(i).getLongitude()));
            WaypointV2 waypoint = new WaypointV2.Builder()
                    .setDampingDistance(20f)
                    .setCoordinate(new LocationCoordinate2D(Double.parseDouble(mission.getWaypoints().get(i).getLatitude()), Double.parseDouble(mission.getWaypoints().get(i).getLongitude())))
                    .setAltitude(Double.valueOf(mission.getWaypoints().get(i).getAltitude()))
                    ...
                    .setTurnMode(WaypointV2MissionTypes.WaypointV2TurnMode.CLOCKWISE)
                    .setAutoFlightSpeed(Float.parseFloat(mission.getWaypoints().get(i).getSpeed()))
                    .build();
        ...
        waypointV2MissionBuilder.setMissionID(new Random().nextInt(65535))
                .setMaxFlightSpeed(Float.parseFloat(mission.getSpeed()))
                .setAutoFlightSpeed(Float.parseFloat(mission.getSpeed()))
...
//                            sendCorrectMsg2Server(message, "loadMission success：uploading...");
                        uploadWayPointV2Mission(mission);
                    } else {
                        Logger.e("mission loadMission fail：" + djiWaypointV2Error.getDescription());
                    }
                }
            });
        } else {
            Logger.e("mission loadMission fail：" + waypointMissionOperator.getCurrentState() + "：The mission can be loaded only when the operator state is READY_TO_UPLOAD or READY_TO_EXECUTE");
        }

    }

    //上传航点任务
    private void uploadWayPointV2Mission(MissionUploadData mission) {
...
//                                    e.printStackTrace();
//                                }
//                                startWaypointsV2Mission();
//                            }else{
//                                Logger.e("mission uploadWaypointActions fail:"+djiWaypointV2Error.getDescription());
//                            }
//                        }
//                    });
//                    uploadWayPointV2Action();
                    ...
                    Logger.e("mission uploadMission fail：" + djiWaypointV2Error.getDescription());
                }
            }
        });
    }
    ...
    //上传航点动作
//    private void uploadWayPointV2Action(MissionUploadData mission) {
//        List<WaypointV2Action> waypointV2ActionList = new ArrayList<>();
//        List<MissionUploadData.Waypoint> waypoints = mission.getWaypoints();
//        for (int i = 0; i < waypoints.size(); i++) {
//            MissionUploadData.Waypoint waypoint = waypoints.get(i);
//            for (int j = 0; j < waypoint.get().size(); j++) {
//                actionID += 1;
...
//    }
    ...
    private void startWaypointsV2Mission() {
            ...
            waypointMissionOperator.startMission(new CommonCallbacks.CompletionCallback<DJIWaypointV2Error>() {
                ...
                public void onResult(DJIWaypointV2Error djiWaypointV2Error) {
                    if (djiWaypointV2Error == null) {
                        ...
                        Logger.e("mission uploadAction fail：调用起飞失败：" + djiWaypointV2Error.getDescription());
                    }
                }
            });
        } else {
            Logger.e("startMission fail：" + waypointMissionOperator.getCurrentState());
        }
    }
}
